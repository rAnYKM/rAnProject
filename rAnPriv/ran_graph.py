...
class RanGraph:
    ...
    *ATTENTION: PLEASE AVOID DUPLICATE SOCIAL NODE AND ATTRIBUTE NAME*
    ...
    def single_protection(self, uid, secrets, price, epsilon, mode='greedy', p_mode='single'):
        attr_edge = []
        if len(secrets) == 0:
            attr_edge += [attr for attr in self.soc_attr_net.neighbors(uid) if attr[0] == 'a']
        else:
            eps = epsilon
            fn = [i for i in self.soc_attr_net.neighbors(uid)
                 if i[0] == 'a' and i not in secrets]
            items = list()
            for a in fn:
                ...
                if p_mode == 'single':
                    ...
                    items.append((a, price[uid][a], weight))
